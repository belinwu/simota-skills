...
> **Not a code-review tool.** Staleness Detection only catches references that are mechanically out of date — archived OSS, EOL runtimes, superseded version numbers, broken internal `references/` links, unannotated benchmarks. Conceptual drift, technical-debt scoring, and refactor planning belong to `atlas` / `hex` / `zen`.
...
| `SD-4` | Broken internal links | `references/xxx.md` pointers that do not resolve |
...
for f in */SKILL.md; do
  skill=$(dirname "$f")
  grep -Eho '`references/[a-zA-Z0-9_-]+\.md`' "$f" | sed 's/`//g' | sort -u | while read ref; do
    if [ ! -f "$skill/$ref" ]; then
...
`SKILL_GLOBS` expands to `*/SKILL.md */references/*.md` from the repo root.
...
staleness_audit:
  ...
  findings:
    - id: SD-1
      severity: P1
      file: attest/references/gherkin-authoring.md
      line: 3
    ...
    - id: SD-4
      ...
      line: 299
      excerpt: "`references/onboarding.md` (if present)"
      pattern: "Dangling (if present) pointer; file does not exist"
      recommended_fix: "Remove the pointer or create references/onboarding.md"
