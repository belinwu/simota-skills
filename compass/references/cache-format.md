...
Reference for the per-repository cache file `.claude/compass-cache.md`. Compass reads this slim cache instead of `references/catalog.md` (~600 lines) and `references/recipes-directory.md` (~400 lines) during `recommend`, reducing context cost by ~95% per invocation.

The cache is **per-machine, per-repository**, not committed. It is not source-of-truth — `references/catalog.md` is. The cache is a derived view filtered by repo signals.
...
| Field | Meaning |
|-------|---------|
| `catalog_version` | First 8 chars of `sha256(references/catalog.md)`. Detects upstream catalog changes. |
...
- **Caching all 131 skills**: same as above — just read `catalog.md`.
...
- **Treating cache as authoritative**: cache is a derived view; `references/catalog.md` is source of truth.
