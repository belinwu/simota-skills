...
> **CRITICAL: Bard（Claude main context）は投稿テキストを直接生成してはならない。** すべての投稿テキストは外部エンジンまたは Claude subagent（Task tool）経由で生成すること。これにより各エンジン固有の語彙・思考パターンがペルソナの声に反映される。

| Persona | Engine | Fallback |
|---------|--------|----------|
| Codex | `codex exec --full-auto` | Claude subagent (Task tool) |
| Gemini | `gemini -p --yolo` | Claude subagent (Task tool) |
| Claude | Claude subagent (Task tool) | — |

### Step 0: Availability Check (MUST)

投稿生成の前に**必ず**エンジンの可用性を確認する:
...
結果をログに記録し、unavailable の場合のみ Claude subagent にフォールバック。
...
Pass character sketch + git data + example path via Task (`subagent_type: general-purpose`, `mode: dontAsk`). Claude subagent can read files itself. **Bard main context が直接テキストを書くのではなく、必ず Task tool で subagent を起動すること。**

### Dispatch: Crosstalk (Multi-Post)

Multi-Post Crosstalk では各 Post を担当ペルソナのエンジンで**個別に**生成する:

1. **Post 1** → Primary persona のエンジンで生成（git data のみ渡す）
2. **Post 2** → Secondary persona のエンジンで生成（git data + Post 1 テキストを渡す）
3. **Post 3**（3人の場合）→ Tertiary persona のエンジンで生成（git data + Post 1-2 テキストを渡す）

各 Post 間で前の Post テキストを次のエンジンに渡すことで、自然なリアクションチェーンが生まれる。
