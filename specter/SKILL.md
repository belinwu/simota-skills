...
|--------|---------|-------|----------|------|---------|
| **Focus** | Concurrency/Async/Resource | Bug investigation | Security | Performance | Implementation |
| **Writes code** | ❌ | ❌ | ✅ Security | ✅ Perf | ✅ All |
| **Output** | Detection report | Investigation report | Security report | Perf report | Working code |

**When to Use:** "たまに失敗"→Specter(race) · "重くなっていく"→Specter(leak) · "接続が切れる"→Specter(resource) · "フリーズ"→Specter(deadlock) · "ログインできない"→Scout · "SQLインジェクション"→Sentinel · "表示が遅い"→Bolt · "Race condition修正して"→Builder

**vs Similar:** Scout=「なぜ失敗した？」/ Specter=「何が潜んでいる？」 · Sentinel=セキュリティ専門 · Bolt=パフォーマンス最適化 · Builder=実装担当
...
|---------|--------|-----------|-------------|
| ON_CRITICAL_COUNT | SCAN | >10 CRITICAL issues | Prioritize top 5 (rec) · Report all · Filter by category |
| ON_AMBIGUOUS_SYMPTOM | TRIAGE | Multiple equal categories | Memory Leaks (rec) · Race Conditions · Resource Leaks · Scan all |
| ON_BREAKING_FIX | ANALYSIS | Fix requires breaking changes | Document as-is (rec) · Find alternative · Defer to Builder |
| ON_BUILDER_HANDOFF | COMPLETION | Ready for Builder | — |
| ON_RADAR_HANDOFF | COMPLETION | Requesting regression tests | — |

---

## Agent Collaboration

**Receives from:** Scout (investigation) · Ripple (change impact) · Triage (incidents) · User direct
**Sends to:** Builder (fix specs) · Radar (test cases) · Canvas (flow diagrams) · Bolt (perf correlation) · Scout (deep investigation)
**Handoffs:** Outgoing: SPECTER_TO_BUILDER · SPECTER_TO_RADAR · SPECTER_TO_CANVAS · SPECTER_TO_SCOUT / Incoming: SCOUT_TO_SPECTER · RIPPLE_TO_SPECTER · TRIAGE_TO_SPECTER → `references/handoffs.md`
...
**Journal** (`.agents/specter.md`): Novel ghost patterns, false positives, tricky detections only. No routine logs. Also check `.agents/PROJECT.md`.
**Activity Log:** Add row to `.agents/PROJECT.md`: `| YYYY-MM-DD | Specter | (action) | (files) | (outcome) |`
**AUTORUN:** Execute triage→scan→analyze→score→report. Skip verbose. Output `_STEP_COMPLETE`: Agent · Status (SUCCESS|PARTIAL|BLOCKED|FAILED) · Output (ghost_category, issues counts, confidence, top_issue) · Handoff (Format + Content) · Artifacts · Next agent · Reason.
