...
Because Phase 3 is a large homogeneous parallel sweep, it is a natural candidate for a **native Dynamic Workflow** execution substrate (`references/managed-agents-mapping.md` §5) when available — Nexus stays the routing/recipe layer (which tracks, what feature_id contract) and delegates the parallel track execution. Fall back to L2 parallel spawn otherwise.
...
| Testing | 10 | `drill` (`test_cases.csv` TC-001…, mapped to feature_id) ‖ `radar`? (strategy) ‖ `mint` (`ai_evaluation_cases.csv`) | test_strategy, test_cases.csv, qa_checklist, ai_evaluation_cases.csv, performance_test_plan, security_test_plan, accessibility_test_plan, release_checklist |
...
Nexus AUTORUN venture idea="<X>" depth=<...> mode=<...>
  ...
  ‖ [Test]   drill(test_cases.csv TC-001 ↔ F-id) ‖ radar? ‖ mint(ai_eval_cases.csv)
