...
> **`venture` is the `startup` preset of this recipe.** `/nexus venture` ≡ `/nexus package domain=startup`. The startup blueprint's full per-file detail lives in `references/venture-recipe.md`; this file is the canonical **engine + preset registry**.
>
...
| Recipe | Relationship |
|--------|--------------|
| `venture` | The `startup` preset. Kept as a named alias for discoverability; dispatches to `package domain=startup`. Full blueprint in `references/venture-recipe.md`. |
...
|--------|-----------|-------------|------------------------|------------------------|-----------|
| **startup** | `package domain=startup` / `venture` | 00_overview … 13_assets (14) | accord+spark+rank+pulse | vision/muse/prose/tone ‖ palette/canvas/echo ‖ funnel ‖ pulse/experiment ‖ atlas/schema/gateway/beacon/gear/crypt ‖ oracle ‖ clause/cloak/comply/omen/ripple ‖ drill/radar/mint ‖ sherpa/scribe ‖ sketch/canvas | standard |
| **generic** | `package domain=generic` | 00_overview,01_research,02_strategy,03_design,04_execution,05_assets,06_evaluation,07_operations | accord+spark+rank | researcher ‖ accord/canvas ‖ sherpa/scribe ‖ mint ‖ drill ‖ omen+ripple | standard |
...
| **ai-adoption** | `package domain=ai-adoption` | 00_overview,01_use_cases,02_data,03_prompt,04_system,05_policy,06_training,07_evaluation,08_rollout | accord(use_cases)+spark+rank | oracle(prompts/RAG/eval) ‖ cloak+sentinel(data/security) ‖ schema(data) ‖ scribe(policy/training) ‖ gear(rollout) ‖ drill+mint(eval+hallucination cases) ‖ comply(governance) | hallucination + data-sensitivity + human-in-loop mandatory |
...
| **learning** | `package domain=learning` | 00_learning_goal,01_curriculum,02_materials,03_assessment,04_support,05_progress,06_instructor | mentor(objectives → curriculum, Bloom's alignment owner) | mentor(curriculum/assessment) ‖ researcher(topic) ‖ canvas(learning-path) ‖ morph(format) ‖ drill(hands-on practice) ‖ scribe(formal syllabus) | ALIGNMENT_GAP / ORPHAN_ASSESSMENT; regulated domain (medical/legal/finance/safety/certification) → official-syllabus confirmation |
...
### startup
Full 14-directory blueprint with per-file → agent mapping: **`references/venture-recipe.md`**. Distinctive: one_page_pitch, design_tokens.json, index.html LP, database_schema.sql, api_design_openapi.yaml, test_cases.csv, backlog.csv, growth_experiments.md, risk_register.md.
