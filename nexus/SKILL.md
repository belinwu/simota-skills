...
| `references/package-recipe.md` | `/nexus package` — generalized document-package generator (canonical engine + domain-preset registry). Shared domain-agnostic Phase 0-6 engine, generalized entity-id barrier (F-/H-/UC-/R-/P-/E- per preset), 8 presets (startup/generic/research/ai-adoption/legal/saas/media/growth) with directory + spine + track + risk-gate blueprints, theme→preset auto-detection table + precedence rules + confidence gate, legal/ai-adoption mandatory risk gates, coverage map vs source collection + deferred domains (career/learning/hiring/local-gov → generic fallback), decision tree vs venture/apex/kaizen/growth-acceptance/deep-research |
