...
| Acceptance | `acceptance` | | **Proof-Carrying PR pipeline** for Tier-S/A changes. Orchestrates the 5-layer machine-adjudicated merge regime defined in `_common/PROOF_CARRYING.md` (spec graph → oracle generation → adversarial exploration → Acceptance Gate → runtime oracle hookup). Use when a change needs merge without human visual confirmation (payment / medical / PII / auth / core revenue). **Tier-based scope** (S=14-22 agents, A=8-14 agents; Tier-B/C auto-downgrade to `feature`). Cross-engine diversity is mandatory for Tier-S (G1). Includes Hot-Fix Fast-Path, Success-PR random review (G2), and repair-loop circuit breaker (G3). | Phase 0: Nexus[tier-classify] → Phase 1: attest[spec-diff] (+accord/scribe) → Phase 2 (parallel, agy for Tier-S): radar ‖ mint ‖ drill ‖ voyager? ‖ sentinel ‖ attest[contracts] → Phase 3 (parallel, claude for Tier-S): vigil ‖ voyager+navigator? ‖ sentinel ‖ specter ‖ siege? → Phase 4: judge[tri-engine] → attest[conformance] → guardian[PR+evidence] → Phase 5 (on PASS): beacon[runtime oracle] → mend[repair runbook+circuit breaker] → Phase 6 (async): sample 5%S/2%A → human review |
...
- `acceptance`: **Proof-Carrying PR pipeline** for machine-adjudicated merges. Read `_common/PROOF_CARRYING.md` (mandatory — protocol) and `references/acceptance-recipe.md` (phase contracts, chain template, failure escalation, cost profile). **Phase 0 (Tier Classification)**: Nexus internal — inspect touched paths (auth/, billing/, payment/, pii/ → Tier-S), user-declared scope, and `.agents/PROJECT.md` Tier policy if present. Output `tier: S | A | B | C`. If C, abort with `feature` recommendation (acceptance is over-scope). **Phase 1 (Spec Diff, sequential)**: attest[spec-diff] (+accord if spec amendments needed, +scribe if human-readable annotation required). Meta-invariants gate: no unreachable FSM nodes, all DB columns referenced exist, every API contract has at least one consumer test. **Phase 2 (Oracle Generation, parallel, engine=agy for Tier-S per G1)**: radar[property+regression] ‖ mint[fixtures] ‖ drill[E2E] ‖ voyager[UI flows if ui_dimension != none] ‖ sentinel[SAST+sec regression] ‖ attest[contracts if not Phase 1]. Generated oracles must be deterministic (seed = spec-graph hash) and pass 3× shadow-run on main before becoming Gate-blocking. **Phase 3 (Adversarial Exploration, parallel, engine=claude for Tier-S per G1)**: vigil[security attacker] ‖ voyager+navigator[adversarial users if ui_dimension != none] ‖ sentinel[attack surface] ‖ specter[concurrency edges] ‖ siege[load+chaos, Tier-S only]. Each adversarial agent must produce non-trivial exploration log; empty findings without log = rejected (semantic-emptiness rule). **Phase 4 (Acceptance Gate, sequential)**: judge[tri-engine evidence audit] → attest[final conformance verdict] → guardian[PR with embedded evidence package]. Decision rules: (1) all 12 required evidence fields present and non-empty per `PROOF_CARRYING.md`, (2) spec changes pass meta-oracle, (3) cross-engine quorum 2-of-3 for Tier-S, (4) unspecifiable-quality flags (brand/ethics/dark-pattern) route to human regardless of Tier, (5) per-PR compute cap not exceeded. **Phase 5 (Runtime Oracle Hookup, sequential, on PASS only)**: beacon[register rollback_condition as live SLO] → mend[register repair runbook with G3 circuit breaker — same-signature 3/24h cap, 7d escalation]. **Phase 6 (Random Sampling Audit, async post-merge, non-blocking)**: deterministic dice (seed = PR ID + date) at 5% for Tier-S / 2% for Tier-A; if sampled, file human-review task with full evidence package. Audits the Gate, not the change. **Hot-Fix Fast-Path**: P0/P1 declared by triage → downgrade Tier-S→A and Tier-A→B evidence requirements; normal-Gate follow-up PR required within 24h or auto-pages team. Prevents unofficial bypass invention. **Engine routing for Tier-S (G1 mandatory)**: implementer=Codex, oracle-generator=agy, adversarial=Claude — different families to defeat correlated LLM failure. **Anti-patterns prevented**: (1) running on Tier-C scope (Phase 0 aborts), (2) single-engine evidence on Tier-S (Phase 4 quorum check), (3) "no findings" passing Gate (Phase 3 exploration log required), (4) auto-repair infinite loop (G3 circuit breaker), (5) reviewer atrophy (G2 random sampling), (6) spec trusted because it parses (Phase 1 meta-oracle), (7) unspecifiable quality silently approved (Phase 4 carve-out). +Specter for concurrency-heavy changes, +Siege for Tier-S only (load/chaos), +Ripple for blast-radius pre-check. Use for Tier-S/A merges where the organization has committed to the Proof-Carrying PR regime; for one-off changes recommend `feature` or `apex`. **Cost profile**: Tier-S 6-12× `feature`, Tier-A 3-6× `feature`. **Confirm with user before Tier-S launch.**
