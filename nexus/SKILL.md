...
| Essential | `essential` | | Must-have feature discovery: identify the minimum feature set without which the product cannot exist. Subtraction-oriented (MVP definition, core feature identification, over-engineering elimination). Sequential refinement funnel — each step deliberately narrows the previous output. | Plea → Spark → Magi → Rank |
| Killer | `killer` | | Killer feature discovery: identify decisive differentiators that win the market. Addition-and-leap-oriented (competitive gaps, WOW experience, market-winning features). Hub-spoke fan-out for the investigation phase to preserve perspective independence — sequential investigation contaminates Flux's reframe and Plea's latent-need extraction with prior agents' framing. | (Compete ‖ Flux ‖ Plea) → Spark → Magi |
...
| `ESSENTIAL` | Plea → Spark → Magi → Rank (sequential refinement funnel) | `+Void` for aggressive scope cut, `+Accord` for atomic-unit specs |
| `KILLER` | (Compete ‖ Flux ‖ Plea) → Spark → Magi (hub-spoke parallel Phase 1, sequential Phase 2) | `+Riff` for iterative deep-dive, `+Researcher` for market trend grounding in Phase 1 |
