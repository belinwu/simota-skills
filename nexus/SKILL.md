...
| `venture`, `business plan`, `business documentation package`, `document package`, `MVP dossier`, `startup dossier`, `pitch package`, `business-prep package`, `investor material bundle`, `business idea to docs`, `comprehensive product docs` | `venture` |
...
| `references/venture-recipe.md` | `/nexus venture` — business documentation package generator. Depth tiers (lite/mvp/raise/full) × mode overlays (mvp-dev/fundraising/b2b-saas/b2c-growth/ai-product), 14-directory output spec, the Phase 2 feature_id barrier (traceability anchor: F-001/TC-001/BL-001 propagation), per-directory file→agent mapping, multi-format syntax validation, zip packaging, WebSearch-grounded research with research_todo.md fallback, decision tree vs apex/accord/funnel/deep-research |
