...
| Charter | `charter` | **Repo-wide analysis → self-driving Charter incl. team design.** Document-first; stops at durable `docs/CHARTER.md` (no execution). Designs **multi-engine orchestration** (Claude Code plan/design + Codex CLI build loops). Pair with `enact` to run it. 5-15 agents. | See `reference/recipes-detail.md` | `reference/recipes-detail.md`, `reference/charter-recipe.md` |
| Enact | `enact` | **Execute a Charter end-to-end** — construct the team from §5 roster, orchestrate §4 work breakdown, verify, ship. **Runs to completion**: drives every package to a terminal state, no mid-run stops except §8 safety red lines (L4/destructive/out-of-scope); announce-and-proceed. `enact <path>` / `dry-run` / `resume`. 6-30+ agents. | See `reference/recipes-detail.md` | `reference/recipes-detail.md`, `reference/enact-recipe.md` |
