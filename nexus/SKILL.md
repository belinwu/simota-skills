...
| `IOS_UI_TEST` | **Snap** → Gear → Launch | XCUITest authoring, accessibility-identifier audit, programmatic screenshot capture, fastlane snapshot for App Store. `+Native` when defects route back to the shipping app or identifiers must be retrofitted. `+Voyager` for Android parity (Espresso / Compose UI Test / Maestro). `+Judge` for test-quality review. **Pure XCUITest scope** — Appium / Detox / Maestro authoring stays with Voyager. Trigger: "XCUITest", "iOS UI test", "App Store screenshot 自動化", "fastlane snapshot", "accessibilityIdentifier 整備" |
