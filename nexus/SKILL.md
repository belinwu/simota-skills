...
| Essential | `essential` | | Must-have feature **verdict** (not enumeration): converge on **THE ONE** feature without which the product cannot exist, then present as a binary Y/N decision to the user. Subtraction-oriented (MVP definition, core feature identification, over-engineering elimination). Sequential refinement funnel that **terminates in a single decisive recommendation** — rejected alternatives are surfaced with one-line reasons for transparency. | Plea → Spark → Magi → Rank → AskUserQuestion[Y/N] |
| Killer | `killer` | | Killer feature **verdict** (not enumeration): converge on **THE ONE** decisive differentiator, then present as a binary Y/N decision to the user. Addition-and-leap-oriented (competitive gaps, WOW experience, market-winning features). Hub-spoke fan-out for the investigation phase to preserve perspective independence — terminates in a single decisive recommendation (rejected alternatives surfaced with one-line reasons). | (Compete ‖ Flux ‖ Plea) → Spark → Magi → AskUserQuestion[Y/N] |
| Summit | `summit` | | Quality-maximizing tri-engine, four-team mobilization for strategic decisions and high-stakes releases. Mobilizes Claude + Codex + Antigravity (agy) across Analysis / Execution / Verification / Improvement teams with cross-engine quorum and PDCA loop. **agy is a hard prerequisite** — refuses to launch if unavailable. 20-50 agents, 49-158 min wall time, 8-22× cost vs `feature`. **User confirmation always required.** | Phase 0: Framing (Claude) → Phase 1: Analysis (Claude ‖ arena[codex] ‖ arena[agy]) → Phase 2: Planning (Claude opus) → Phase 3: Execution (arena[COLLABORATE] across 3 engines) → Phase 4: Verification (judge ‖ Radar/Voyager[codex] ‖ Ripple/arena[agy]) → Phase 5: Improvement (orbit, max 3 loops, magi-arbitrated) → Phase 6: Delivery (Guardian + Launch) |
...
- `essential`: Plea[pain-extraction] → Spark[spec] → Magi[necessity-arbitration] → Rank[MoSCoW-must] → **DELIVER as Y/N verdict** (sequential — deliberate refinement funnel; each step narrows the previous output and parallelization would force redundant re-synthesis). Subtraction-oriented — Magi's Sophia perspective filters out "Should-have" features posing as "Must-have". Failure mode prevented: over-engineering. **Convergence rule**: each agent considers multiple candidates internally but MUST narrow to fewer than the previous step; Rank's MoSCoW output is filtered to **the single top Must-have** (highest necessity score). **Output format** — DELIVER phase emits a Y/N decision card via AskUserQuestion, not a feature list: `## Essential Verdict / Recommended must-have: <single feature>  / Why: <2-3 lines>  / Source of conviction: Plea→Spark→Magi→Rank chain summary  / Considered but rejected: <2-3 alternatives with one-line reasons each>  / → Build this? [Yes / No / Modify]`. Tie-break rule when Rank cannot pick a single winner: defer to Magi's Sophia perspective; if still tied, escalate to user via AskUserQuestion with the tied candidates. +Void for aggressive scope cut, +Accord for atomic-unit specs. Use for MVP definition, pre-PMF core feature identification, or scope reduction.
- `killer`: Phase 1 (parallel hub-spoke, L2 spawn): Compete[gap-analysis] ‖ Flux[reframe] ‖ Plea[latent-needs] — each in an isolated sub-context to preserve perspective independence. Phase 2 (sequential synthesis): Spark[differentiation-spec] aggregates the three independent perspectives → Magi[strategic-go-no-go] → **DELIVER as Y/N verdict**. **Why parallel for Phase 1**: sequential investigation contaminates downstream agents — Flux loses its reframe power once anchored to Compete's gap framing, and Plea's latent-need extraction collapses toward already-surfaced ideas. The independence is load-bearing for the recipe's value. Addition-and-leap-oriented. Failure mode prevented: mundane or already-shipped proposals (would re-emerge if perspectives are serialized). **Convergence rule**: Spark may evaluate multiple differentiator candidates from the three perspectives, but MUST synthesize **the single most decisive killer feature**; Magi delivers a binary Go/No-Go (not a ranked list). **Output format** — DELIVER phase emits a Y/N decision card via AskUserQuestion, not a candidate matrix: `## Killer Verdict / Recommended killer feature: <single feature>  / Why this wins: <2-3 lines>  / Source of conviction: Compete‖Flux‖Plea→Spark→Magi chain summary with engine-attributed evidence  / Considered but rejected: <2-3 alternatives with one-line reasons each>  / Magi verdict: GO (confidence: H/M/L) | NO-GO (reason)  / → Ship this? [Yes / No / Modify]`. Tie-break rule when Spark cannot identify a single winner: Magi forces selection via strategic-impact criterion (market timing × differentiation depth × feasibility); if Magi returns NO-GO, the recipe still surfaces the runner-up with a "weakest-link" annotation so the user can choose to pursue it. +Riff for iterative deep-dive on Spark output, +Researcher for market trend grounding in Phase 1. Use for growth-phase differentiation, competitive response, or WOW-experience design.
- `summit`: Tri-engine (Claude + Codex + Antigravity), four-team (Analysis / Execution / Verification / Improvement) quality-maximization recipe. Read `references/summit-recipe.md` for full phase contracts, engine × team matrix, cross-engine quorum rules, AUTORUN chain template, failure escalation, and cost profile. **Hard prerequisites**: agy CLI reachable at preflight (no dual-engine fallback — Summit aborts and recommends `apex` if agy is missing), codex CLI reachable, arena skill available, user cost acknowledgment. **Engine isolation**: Claude is always hub; Codex/agy invocations go exclusively through `arena` (never direct Bash). **Cross-engine quorum**: CONFIRMED (3/3) / LIKELY (2/3) / CANDIDATE (1/3, grounded by Nexus) / MINORITY (1/3, rejected by others). **Improvement loop**: orbit-driven, max 3 loops, magi-arbitrated, Agent Tennis circuit breaker. **Scale**: 20-50 agents, 49-158 min wall time, 8-22× cost vs `feature`. **Confirm with user before launch — same gate as apex.** Use for strategic decisions, release-critical verification, or large refactors where blind spots compound. Do NOT use for routine features (use `feature`/`apex`), PR review (use `judge`), or cost-sensitive contexts.
...
| `essential`, `must-have`, `MVP definition`, `core feature`, `minimum viable`, `cut scope`, `bare minimum` | Essential feature verdict chain (subtraction-oriented, converges to single recommendation) | **Single must-have verdict + Y/N decision prompt** (rejected alternatives surfaced with one-line reasons) | (inline in `## Subcommand Dispatch`) |
| `killer`, `killer feature`, `differentiator`, `WOW experience`, `decisive feature`, `competitive edge`, `market-winning` | Killer feature verdict chain (addition-and-leap-oriented, converges to single recommendation) | **Single killer feature verdict + Y/N decision prompt** (rejected alternatives surfaced with one-line reasons) | (inline in `## Subcommand Dispatch`) |
| `summit`, `tri-engine`, `all engines`, `claude+codex+agy`, `quality maximization`, `strategic decision`, `release-critical`, `mobilize all teams` | Summit tri-engine four-team quality-maximization chain | Engine-attributed deliverable + quorum verdict + improvement loop summary | `references/summit-recipe.md` |
...
| `ESSENTIAL` | Plea → Spark → Magi → Rank → Y/N verdict (sequential refinement funnel; **converges to single must-have**) | `+Void` for aggressive scope cut, `+Accord` for atomic-unit specs |
| `KILLER` | (Compete ‖ Flux ‖ Plea) → Spark → Magi → Y/N verdict (hub-spoke parallel Phase 1, sequential synthesis; **converges to single killer feature**) | `+Riff` for iterative deep-dive, `+Researcher` for market trend grounding in Phase 1 |
...
| `references/summit-recipe.md` | User invoked `/nexus summit` or asks for tri-engine quality-maximization (Claude + Codex + Antigravity) across four teams (Analysis / Execution / Verification / Improvement). Read for prerequisites (agy hard requirement), engine × team matrix, phase contracts (0-6), arena sub-orchestration patterns, cross-engine quorum rules (CONFIRMED / LIKELY / CANDIDATE / MINORITY), AUTORUN chain template, failure escalation (no silent dual-engine fallback), cost/latency profile (20-50 agents, 49-158 min, 8-22× cost vs `feature`), and decision tree vs apex/judge. |
