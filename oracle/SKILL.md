---
name: oracle
description: "AI/ML design and evaluation specialist. Covers prompt engineering, RAG design, LLM application patterns, AI safety, evaluation frameworks, MLOps, and cost optimization."
---
...
| Area         | Rule                                                                                                                                  |
| ------------ | ------------------------------------------------------------------------------------------------------------------------------------- |
| Prompt       | use `3-5` few-shot examples only when they measurably help; prefer constrained decoding for structured outputs (reduces iteration rate from `38.5%` to `12.3%`); for Claude, use XML tags (`<instructions>`, `<context>`, `<examples>`) over Markdown for unambiguous parsing; structure prompts for caching: static content first, variable last (`45-80%` cost / `13-31%` TTFT reduction via prompt caching); for Claude 4+, configure adaptive thinking (`thinking: {type: "adaptive"}`) with effort tuned to task complexity — agentic multi-step loops benefit most, simple extraction tasks waste tokens on unnecessary reasoning |
...
| Cost         | budget alert `> 120%`; wasted-token cost target `< 5%`; model routing dispatches to cheapest adequate model (`87%` cost reduction, premium models handle only `~10%` of queries); semantic cache: similarity threshold `>= 0.8`, hit rate target `>= 60%` (practical range `61-69%`, up to `73%` cost reduction in high-repetition workloads); prompt caching: static prefix first (`45-80%` cost savings); combined techniques deliver `70-90%` total savings |
