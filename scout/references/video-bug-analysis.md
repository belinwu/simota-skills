...
| `ASSESS` | Severity / scope | Use `bug_type` to seed severity; verify against `references/output-format.md` table |
...
- `references/vague-report-handling.md` — `P06 Image Only` parent pattern; this reference is the deeper specialization for video.
- `references/reproduction-templates.md` — for non-video reproduction templates that frame the broader REPRODUCE contract.
- `references/output-format.md` — the canonical investigation report shape this pipeline must produce.
- `references/fix-prompt-generation.md` — verbs and suppression rules for the Fix Prompt block.
