...
**Adapted from `judge/references/tri-engine-review.md`. Re-uses PREFLIGHT and FAN-OUT mechanics; replaces SCORE/GROUND/FILTER with Concurrence-Divergence Scoring and Synthesis that preserves persona-voice fidelity.**
...
- Generation mode (EXPLORE / DEEP / CHALLENGE / COMPETE / EDGE) per `references/mode-playbooks.md`
...
- Fill the `PERSONA_CHANNEL` template for each before FAN-OUT (per `references/persona-embodiment.md`).
...
Use the combined preflight from `judge/references/tri-engine-review.md §2`. The probe order is identical for `codex`, `agy`, `claude`.
...
| `demand-agy` | Antigravity CLI | `agy -p "<prompt>" --dangerously-skip-permissions` |
...
4. **Real-data calibration** — if Voice / Trace / Researcher data is available in the project (e.g., `.agents/voice.md`), cross-check whether the synthetic demand aligns with real signals. Apply the confidence tags from `references/calibration.md`:
...
- Mode bias: {EXPLORE | DEEP | CHALLENGE | COMPETE | EDGE — per `references/mode-playbooks.md`}
...
| Fewer than 3 personas available | Multi mode still runs but with the same persona pool — flag persona-representativeness as a risk per `references/calibration.md` |
...
- `plea/references/persona-embodiment.md` — PERSONA_CHANNEL template fed into FAN-OUT
- `plea/references/calibration.md` — confidence tagging applied at CALIBRATE step
- `plea/references/mode-playbooks.md` — Mode Modifier semantics (COMPETE / EDGE / CHALLENGE compatibility)
