...
- Pair every demand and every report with an LLM instruction prompt (per-request + per-report orchestration). Templates and authoring rules: `references/llm-prompt-generation.md`.
...
- Tag every output `synthetic: true` unless calibrated per `references/calibration.md`
...
[Per-request prompt — full template in `references/llm-prompt-generation.md`]
````
...
Each prompt declares **one action verb** at the top of `# Your task`: `ANALYZE` · `PROPOSE` · `DESIGN` · `DRAFT-SPEC` · `PROTOTYPE` · `REFINE`. Default verb by receiving agent, full prompt templates, and authoring rules: `references/llm-prompt-generation.md`.
...
- `roleplay`: DEEP, single-persona embodiment (worst-day, competitor comparison, silent-majority lens). Playbook: `references/persona-embodiment.md`.
- `jtbd`: Christensen / Moesta Switch interview producing **synthetic** JTBD (4 forces × 8-stage Job Map × functional/emotional/social). Real-user JTBD is Researcher's domain — tag `synthetic: true` and hand off. Protocol: `references/jtbd-switch-interview.md`.
- `5whys`: Toyota / Ohno 5 Whys on a user demand. ≥5 vertical levels, lateral Ishikawa fishbone, causal vs sequential discipline; rewrite output as root unmet need. Protocol: `references/5whys-root-cause.md`.
- `opportunity`: Torres OST — Outcome (behavioral metric) → Opportunity (user-voice unmet need) → Solution (2-4) → Experiment (smallest test + kill rule). Weekly cadence; hand off to Researcher / Spark / Experiment. Protocol: `references/opportunity-solution-tree.md`.
- `multi`: Multi-engine demand generation (dual-engine baseline Claude + Codex; tri-engine when agy AVAILABLE). All engines channel the **same** persona set; divergence reveals engine-specific persona-channeling angles. Per-cluster scoring (`UNIVERSAL-DEMAND` 3/3, `LIKELY-DEMAND` 2/3, `VERIFIED-DIVERGENT-VOICE` 1/3) + cross-persona axis (`CROSS-PERSONA-UNIVERSAL`). Compatible with `COMPETE` / `EDGE` / `CHALLENGE`. Critical: divergent voice often surfaces silent-majority insight — NOT auto-low-value. Full flow + JSON schema + degraded modes: `references/tri-engine-demand.md`.
...
[Full template in `references/llm-prompt-generation.md`]
````
...
| `references/llm-prompt-generation.md` | You are authoring per-request or per-report LLM Instruction Prompts — action-verb table, default verb by receiving agent, authoring rules, and full prompt templates (per-request + per-report orchestration) |
...
**Core mechanics:**
- PREFLIGHT in Plea main context (never delegate — subagent PATH is narrower; canonical probe in `judge/references/tri-engine-review.md §2`).
...
Full algorithm, engine-attribution tag matrix, JSON schema, subagent prompt skeletons, calibration rules, and degraded-mode matrix: `references/tri-engine-demand.md`.
...
_STEP_COMPLETE:
  ...
  Output:
    ...
    tri_engine: [present only on `multi` — schema in `references/tri-engine-demand.md`]
  Handoff:
