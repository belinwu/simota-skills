...
`SCOPE → PREFLIGHT → FAN-OUT → NORMALIZE → CLUSTER → SCORE → GROUND → ARBITRATE → FILTER → REPORT` per `references/tri-engine-scan.md`. PREFLIGHT runs in Sentinel main context only — never delegated to subagents, whose inherited PATH is narrower than the user's interactive shell.
...
2. `references/tri-engine-scan.md` — Sentinel JSON schema, CLUSTER identity rules, SCORE rubric, GROUND checks, SYNTHESIZE merge, subagent prompt skeleton
