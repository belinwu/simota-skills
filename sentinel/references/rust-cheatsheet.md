...
> - Constructive security stack: [`builder/references/rust-best-practices.md` §7 Security practices](../../builder/references/rust-best-practices.md#7-security-practices)
> - Misuse / pitfall catalog (12.1–12.20 + 12.A–12.D): [`builder/references/rust-anti-patterns.md` §12 Security Pitfalls](../../builder/references/rust-anti-patterns.md#12-security-pitfalls)
> - `unsafe` UB catalog (9.1–9.20 + 9.A–9.C): [`builder/references/rust-anti-patterns.md` §9 Unsafe Pitfalls](../../builder/references/rust-anti-patterns.md#9-unsafe-pitfalls-high-stakes)
> - Canonical UB list, Stacked vs Tree Borrows, SAFETY-comment template: [`builder/references/rust-language-spec.md` §6 Unsafe Rust](../../builder/references/rust-language-spec.md#6-unsafe-rust)
> - FFI / ABI rules: [`builder/references/rust-language-spec.md` §8 FFI / Interop](../../builder/references/rust-language-spec.md#8-ffi--interop)
...
Every `unsafe { ... }` block must have a `// SAFETY:` comment naming each invariant the caller upholds (template: [language-spec §6.5](../../builder/references/rust-language-spec.md#65-safety-comment-template)). Flag any block missing one — enforce with `clippy::undocumented_unsafe_blocks`.

Walk these patterns in order; each links to the full anti-pattern entry:

1. **Transmute layout assumptions** — `mem::transmute` between non-`#[repr(transparent)]` types is UB even if fields match. See [anti-patterns §9.1](../../builder/references/rust-anti-patterns.md#9-unsafe-pitfalls-high-stakes), §9.15, §9.B.
...
|------|-------|-----------|
| Panics MUST NOT cross an `extern "C"` boundary (UB) | Every `#[unsafe(no_mangle)] pub extern "C" fn` body wrapped in `std::panic::catch_unwind`, OR the function is declared `extern "C-unwind"` and the C side handles it | [language-spec §8](../../builder/references/rust-language-spec.md#8-ffi--interop), [anti-patterns §9.8, §9.C](../../builder/references/rust-anti-patterns.md#9-unsafe-pitfalls-high-stakes) |
...
| Check | grep / lint | Why |
|-------|-------------|-----|
| 4.1 `Debug` derive on a struct holding secrets | `rg '#\[derive\(.*Debug.*\)\].*\n.*(password\|token\|secret\|key)' --type rust -A 3` | `{:?}` prints secrets into logs and panic messages. See [anti-patterns §12.20](../../builder/references/rust-anti-patterns.md#12-security-pitfalls). |
| 4.2 Secret types not wrapped in `secrecy::SecretBox` / `SecretString` | Type-check audit for `String` / `Vec<u8>` named password/token/key | `SecretBox<T>` excludes itself from `Debug`/`Display`/`Serialize`. See [best-practices §7.3](../../builder/references/rust-best-practices.md#7-security-practices). |
...
Full CI YAML lives in [best-practices §7.1](../../builder/references/rust-best-practices.md#7-security-practices). Skeleton:
...
**2026 trusted publishing** (crates.io OIDC, late-2025 rollout): no long-lived `CARGO_REGISTRY_TOKEN` secret — short-lived OIDC token via `rust-lang/crates-io-auth-action@v1`. Adopt for all new publish workflows. See [best-practices §7.8](../../builder/references/rust-best-practices.md#7-security-practices).
...
|-------|------------|-----------|
| **Shell injection** | `rg 'Command::new\("(sh\|bash\|cmd\|powershell)"\)' --type rust` | [anti-patterns §12.5, §12.C](../../builder/references/rust-anti-patterns.md#12-security-pitfalls) |
...
| **Bincode** | **Never** for untrusted input — trusts size prefixes; attacker says `usize::MAX` → OOM. See [anti-patterns §12.6, §12.16](../../builder/references/rust-anti-patterns.md#12-security-pitfalls). |
...
Sentinel does not author crypto — that is Crypt's domain (see [`crypt/references/rust-cheatsheet.md`](../../crypt/references/rust-cheatsheet.md)). The audit-side rules:
...
See [best-practices §7.5, §7.6](../../builder/references/rust-best-practices.md#7-security-practices) for the crate matrix.

### 9. Public API leakage

| Leak | Fix |
|------|-----|
| `Box<dyn Error>` in public API exposes inner types | Concrete `thiserror` enum at crate boundary. See [anti-patterns §5.1](../../builder/references/rust-anti-patterns.md#5-error-handling-pitfalls). |
...
Or use `clippy::arithmetic_side_effects` to flag every `+`/`-`/`*`/`/` outside explicit `checked_*` / `saturating_*` / `wrapping_*` paths. See [anti-patterns §12.9](../../builder/references/rust-anti-patterns.md#12-security-pitfalls).
...
Or use `regex-lite` for tighter resource bounds. See [anti-patterns §12.7](../../builder/references/rust-anti-patterns.md#12-security-pitfalls).
