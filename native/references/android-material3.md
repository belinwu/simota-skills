...
→ Implementation: `SKILL.md` Native Stack Defaults / UI row, `references/modern-stack.md`
...
- Deep links: `navDeepLink<Destination>()` with `App Links` (`assetlinks.json`) — see `references/deeplink-routing.md`.
...
- Never gate core app entry on permission grants — use soft pre-prompt + graceful degradation (see `references/platform-permissions.md`).
...
| Type-safe Navigation Compose 2.8+ | `SKILL.md` Navigation row, `references/patterns.md` |
...
| Material Symbols + variable fonts | `references/patterns.md` |
