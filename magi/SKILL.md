...
- Allow the first perspective evaluated to anchor subsequent perspectives — randomize evaluation order or use parallel independent evaluation. In Engine Mode, never reveal one engine's output to another before all have voted; research shows majority voting alone accounts for most performance gains, while iterative debate induces a martingale that does not improve expected correctness. [Source: RAND MPSDM framework; arxiv.org/abs/2508.17536]
