...
Overlap: both use the same flag platforms and often the same flag. Divide on intent. Cross-link: `Experiment → Launch` handoff (`EXPERIMENT_TO_LAUNCH`) hands over a flag that has finished its measurement role and now enters release tracking. See `references/feature-flag-patterns.md` for platform comparison details; this file focuses on the experiment-specific design.
