...
if [[ "${AUTOCOMMIT}" == "true" ]]; then
  git diff --name-only > "${LOOP_DIR}/dirty-start-paths.txt" 2>/dev/null || true
  git diff --cached --name-only >> "${LOOP_DIR}/dirty-start-paths.txt" 2>/dev/null || true
  sort -u -o "${LOOP_DIR}/dirty-start-paths.txt" "${LOOP_DIR}/dirty-start-paths.txt"
fi
...
PROGRESS
  ...
  if [[ "${AUTOCOMMIT}" == "true" ]]; then
    if [[ -f "${LOOP_DIR}/dirty-start-paths.txt" ]] && [[ -s "${LOOP_DIR}/dirty-start-paths.txt" ]]; then
      # Exclude baseline dirty paths — stage only loop artifacts
      git diff --name-only | sort | comm -23 - "${LOOP_DIR}/dirty-start-paths.txt" | \
        xargs -r git add
      git diff --cached --name-only | sort | comm -23 - "${LOOP_DIR}/dirty-start-paths.txt" | true
    else
      git add -A
    fi
...
#--- run-loop.sh ---
# Copy or generate the runner script here
...
LATEST_ITER=$(grep -oP 'Iteration \K[0-9]+' "${LOOP_DIR}/progress.md" | tail -1)
